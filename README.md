# First Principles

Software engineering, rebuilt from the constraints up.

No folklore. No hand-waving. Just the reasoning that makes the rules make sense.

## Start here

- [No Cap: The Real CAP Theorem](./topics/system-design/cap-theorem.md) — why “our database is AP” is usually wrong, and what CAP actually says.

- [SSH from First Principles](./topics/system-design/ssh-mastery.md) — how public-key cryptography works, what happens during the SSH handshake, and how to run two GitHub identities on one machine.

More notes on distributed systems, databases, networking, and backend engineering as I write them.
